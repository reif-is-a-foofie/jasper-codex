...
- `jasper browser open URL`, `jasper browser run --plan-file browser-plan.json`, and `jasper browser inspect --debug-port DEBUG_PORT` now drive or inspect a real local Chrome automation session for navigation, filling, clicks, snapshots, screenshots, takeover, recovery, and explicit file moves after browser work completes.
