...
#[test]
fn otel_export_routing_policy_routes_log_only_and_trace_safe_events() {
    ...
    let mut log_event_names = logs
        .iter()
        .filter_map(|log| log_attributes(&log.record).get("event.name").cloned())
        .collect::<Vec<_>>();
    log_event_names.sort();
    assert_eq!(
        log_event_names,
        vec![
