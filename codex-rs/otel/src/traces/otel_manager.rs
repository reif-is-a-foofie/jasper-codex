...
macro_rules! emit_trace_safe_event {
    ($self:expr, $($fields:tt)*) => {{
        tracing::event!(
...
fn error_class(error: Option<&str>) -> Option<&'static str> {
    let error = error?;
    ...
    if error.contains("timeout") {
        ...
        Some("forbidden")
    } else if error.contains("stream closed") {
        Some("stream_closed")
...
impl OtelManager {
    ...
    pub fn record_websocket_event(
        ...
        emit_trace_safe_event!(
            ...
            duration_ms = %duration.as_millis(),
            success = success_str,
            error.class = error_class(error_message.as_deref()),
        );
    }
        ...
        T: Display,
    {
        ...
        emit_trace_safe_event!(
            ...
            event.kind = %kind_str,
            duration_ms = %duration.as_millis(),
            error.class = error_class(Some(&error.to_string())),
        );
    }
